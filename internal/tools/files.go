...
func registerFileTools(s *mcp.Server, w *Workspace) {
	mcp.AddTool(s, &mcp.Tool{
		...
		entries, truncated, err := d.Sandbox.List(in.Dir, project.ListOptions{
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		res, err := d.Sandbox.Read(in.Path, in.StartLine, in.MaxLines)
		return nil, res, err
	})
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		created, err := d.Sandbox.Write(in.Path, in.Content, true)
		if err != nil {
			return nil, WriteFileOut{}, err
		}
		return nil, WriteFileOut{
			Path: d.Sandbox.ToRes(abs), Created: created, Bytes: len(in.Content), Hint: hintFor(in.Path, created),
		}, nil
	})
	...
	mcp.AddTool(s, &mcp.Tool{
		...
		n, err := d.Sandbox.Edit(in.Path, in.OldStr, in.NewStr, in.ReplaceAll)
		if err != nil {
			return nil, EditFileOut{}, err
		}
